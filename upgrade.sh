...
# ─────────────────────────────────────────────────────────────────────────────
# 1. 停止服务（必须在备份数据库之前停止，以确保 SQLite WAL 完整 checkpoint）
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# 2. 备份当前版本（保留配置和数据库）
# 注意：必须在服务停止后备份，确保 SQLite WAL 已 checkpoint 到主数据库文件
# ─────────────────────────────────────────────────────────────────────────────
...
if [[ -n "${NEW_SOURCE}" ]] && [[ "${NEW_SOURCE}" != "${INSTALL_DIR}" ]]; then
    info "更新程序文件..."
    SKIP_PATTERNS=(config.json hdy_monitor.db hdy_monitor.db-wal hdy_monitor.db-shm credentials.txt venv)
    RSYNC_EXCLUDES=()
